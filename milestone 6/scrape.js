...
import { sendTelegramMessage, notifyChanges, isEnabled } from './src/notifications.js';
import { startScheduler, runOnce } from './src/scheduler.js';
...
Options:
  ...
  --schedule <cron>          Start scheduler with cron expression (e.g., "*/30 * * * *")
  --once                     Run all configured sites once and exit
  --dashboard                Start web dashboard (default port 3000)
...
Examples:
  ...
  node scrape.js --test-telegram
  node scrape.js --sync
  node scrape.js --once
  node scrape.js --schedule "*/30 * * * *"
  node scrape.js --dashboard
  node scrape.js --dashboard --port 8080
...
async function main() {
  try {
    ...
    // Handle --schedule
    if (values.schedule) {
      const cronExpr = values.schedule || process.env.SCRAPE_CRON || '*/30 * * * *';
      try {
        await startScheduler(cronExpr, ['iparralde'], values['dry-run'] || false);
      } catch (err) {
    ...
    // Handle --once
    if (values.once) {
      const exitCode = await runOnce(['iparralde'], values['dry-run'] || false);
    ...
    if (values['test-telegram']) {
      console.log('Testing Telegram connection...');
      const { testConnection } = await import('./src/notifications.js');
    ...
    if (values.persist || (!values.out && !values['dry-run'])) {
      if (values['dry-run']) {
        ...
        if (runId) {
          ...
          // Send Telegram notification if enabled (Milestone 4)
          if (isEnabled()) {
            await notifyChanges(summary, runId, false);
          }
